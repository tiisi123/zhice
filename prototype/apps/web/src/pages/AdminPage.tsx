...
// Lazy load the two new T07 panels so the邀请码 tab (most-frequent) keeps
// its existing first-paint cost. AdminPage is admin-only so the bundle-size
// cost is small either way; lazy boundary protects against accidental cycle
// regressions in future panel additions.
const AdminKplCookiePanel = lazy(() => import('./AdminKplCookiePanel'))
const AdminHealthPanel = lazy(() => import('./AdminHealthPanel'))
...
function InviteCodesContent() {
  ...
  const generate = async () => {
    setGenerating(true)
    try {
      const r = await postApi<{ codes: string[] }>(
        '/payment/admin/generate-codes',
  ...
  const copyAll = () => {
    ...
    if (!unused.length) {
      message.warning('没有可用的邀请码')
      return
    }
    const text = unused
      .map((c) => `${c.code}  (${c.plan} ${c.days}天)`)
      .join('\n')
            ...
            { title: '天数', dataIndex: 'days', width: 70 },
            {
              ...
              render: (_: unknown, r: InviteCode) => (
                <span
                  style={{
                    color:
                      r.used_count >= r.max_uses ? '#52c41a' : '#999',
                  }}
                >
...
export default function AdminPage() {
  ...
  return (
    <div>
      <Title level={3}>业主后台</Title>
      <Tabs defaultActiveKey="invite-codes" items={items} destroyOnHidden />
    </div>
  )
}
