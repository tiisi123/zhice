...
def portfolio_dashboard(user: dict = Depends(current_user)):
    ...
    if not rows:
        return wrap_contract(
            ...
            status="empty",
            mock=False,
            data_source="user_watchlist",
            data_mode="empty",
            as_of="",
            fallback_reason="no_user_positions",
            stocks=[],
            total_value=0,
            total_pnl=0,
            total_pnl_rate=0,
    ...
    return wrap_contract(
        ...
        mock=False,
        data_source="dfcf",
        data_mode="live" if status == "real" else "unavailable",
        as_of="",
        fallback_reason="" if status == "real" else "dfcf_quote_unavailable",
        stocks=stocks,
...
def meso_tracking(industry: Optional[str] = Query(None)):
    try:
        ...
        return wrap_contract(
            ...
            data_source="static_meso_indicators",
            data_mode="sample" if data else "empty",
            as_of="",
            fallback_reason="real_meso_source_not_connected" if data else "no_static_meso_match",
            count=len(data),
        )
    except Exception as e:
...
def rotation_simulation(source: str = Query("半导体")):
    try:
        ...
        return wrap_contract(
            ...
            data_source="static_rotation_rules",
            data_mode="sample" if targets else "empty",
            as_of="",
            fallback_reason="real_rotation_source_not_connected" if targets else "no_static_rotation_match",
            input_source=source,
            targets=targets,
        )
    except Exception as e:
